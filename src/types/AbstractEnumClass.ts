...
export abstract class AbstractEnumClass implements EnumIO {
  readonly index: number;
  readonly symbol: symbol;
  readonly str: string;
  ...
  constructor({
    ...
    if (AbstractEnumClass.__indices.get(this.constructor) == null) {
      AbstractEnumClass.__indices.set(this.constructor, []);
    }
    if (AbstractEnumClass.__strings.get(this.constructor) == null) {
      AbstractEnumClass.__strings.set(this.constructor, []);
    }
    if (
      AbstractEnumClass.__indices.get(this.constructor)!.indexOf(index) !== -1
    ) {
      throw new Error('Dont use duplicate index.');
    }
    if (noCheckStrUnique !== true) {
      if (
        AbstractEnumClass.__strings.get(this.constructor)!.indexOf(str) !== -1
      ) {
    ...
    AbstractEnumClass.__indices.get(this.constructor)!.push(index);
    AbstractEnumClass.__strings.get(this.constructor)!.push(str);
  }
